...
- (void)layoutSubviews {
  ...
  if (self.isLoading &&
      [self.titleLabel.text isEqualToString:self.loadingText]) {
...
- (void)layoutSubviewsWhenLoadingButtonAlignmentLeft {
  ...
  [self
      setTitleEdgeInsets:UIEdgeInsetsMake(
                             0, 0, 0, -(activityIndicatorViewFrame.size.width +
                                        kSpaceWidth))];
  CGFloat activityIndicatorViewX = 0.0;
  if (!CGRectEqualToRect(self.titleLabel.frame, CGRectZero) &&
      !CGRectEqualToRect(self.imageView.frame, CGRectZero)) {
    activityIndicatorViewX = self.titleLabel.left < self.imageView.left
                                 ? self.titleLabel.left - kSpaceWidth
                                 : self.imageView.left - kSpaceWidth;
  ...
  activityIndicatorViewX =
      activityIndicatorViewX - activityIndicatorViewFrame.size.width;
  if (activityIndicatorViewX - self.loadingButtonEdgeInsets.left < 0) {
    if (self.isHiddenActivityIndicatorWhenLoading) {
      CGFloat maxTitleWidth = self.width - self.loadingButtonEdgeInsets.left -
                              self.loadingButtonEdgeInsets.right;
      CGFloat realTitleWidth =
          [self.titleLabel sizeThatFits:CGSizeMake(maxTitleWidth, CGFLOAT_MAX)]
              .width;
      self.titleLabel.width =
          maxTitleWidth < realTitleWidth ? maxTitleWidth : realTitleWidth;
      ...
      activityIndicatorViewFrame.origin.y =
          (self.height - activityIndicatorViewFrame.size.height) / 2.0;
      self.activityIndicatorView.frame = activityIndicatorViewFrame;
      self.titleLabel.left = self.loadingButtonEdgeInsets.left +
                             activityIndicatorViewFrame.size.width +
                             kSpaceWidth;
...
- (void)layoutSubviewsWhenLoadingButtonAlignmentRight {
  ...
  [self setTitleEdgeInsets:UIEdgeInsetsMake(
                               0, -(activityIndicatorViewFrame.size.width +
                                    kSpaceWidth),
                               0, 0)];
  CGFloat activityIndicatorViewX = 0.0;
  if (!CGRectEqualToRect(self.titleLabel.frame, CGRectZero) &&
    ...
    activityIndicatorViewX =
        self.titleLabel.left > self.imageView.left
            ? self.titleLabel.left + self.titleLabel.width + kSpaceWidth
            : self.imageView.left + self.imageView.width + kSpaceWidth;
  } else if (CGRectEqualToRect(self.titleLabel.frame, CGRectZero)) {
    activityIndicatorViewX =
        self.imageView.left + self.imageView.width + kSpaceWidth;
  } else {
  ...
  if (activityIndicatorViewX + activityIndicatorViewFrame.size.width +
          self.loadingButtonEdgeInsets.right >
      self.width) {
    if (self.isHiddenActivityIndicatorWhenLoading) {
      CGFloat maxTitleWidth = self.width - self.loadingButtonEdgeInsets.left -
      ...
      activityIndicatorViewFrame.origin.x =
          self.width - self.loadingButtonEdgeInsets.right -
          activityIndicatorViewFrame.size.width;
      activityIndicatorViewFrame.origin.y =
          (self.height - activityIndicatorViewFrame.size.height) / 2.0;
      self.activityIndicatorView.frame = activityIndicatorViewFrame;
...
- (void)layoutSubviewsWhenLoadingButtonAlignmentCenter {
  CGRect activityIndicatorViewFrame = self.activityIndicatorView.frame;
  // 不显示图片和文字
  activityIndicatorViewFrame.origin =
      CGPointMake((self.width - activityIndicatorViewFrame.size.width) / 2.0,
                  (self.height - activityIndicatorViewFrame.size.height) / 2.0);
  self.activityIndicatorView.frame = activityIndicatorViewFrame;
}
...
- (void)startLoadingWithLoadingText:(NSString *)loadingText {
  if (!self.isLoading) {
    if (loadingText.length != 0 &&
        self.loadingButtonAlignment != CNLLoadingButtonAlignmentCenter) {
...
- (void)setLoadingButtonAlignment:
    (CNLLoadingButtonAlignment)loadingButtonAlignment {
